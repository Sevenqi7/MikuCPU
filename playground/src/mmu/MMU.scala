...
class LA32AddrTransUnit extends MkModule {
    ...
    la32_tlb.io.read_port.index := DontCare
    la32_tlb.io.write_port      := DontCare
    ...
    for ((ch, idx) <- channels.zipWithIndex) {
        ...
        la32_tlb.io.search_port(idx).req := 0.U.asTypeOf(new TLBSearchReq)

        val tlb_resp = la32_tlb.io.search_port(idx).resp
        vaddr_r(idx) := ch.vaddr
        ch.paddr     := DEBUG_MAGICNUM.U
        ch.tlb_resp  := DontCare
        ch.dmw_hits  := 0.U.asTypeOf(ch.dmw_hits)
        ...
        when(da_mode) {
            ...
            la32_tlb.io.search_port(idx).req := ch.genTLBSearchReq(io.from_csr.asid.ASID, !dmw_hits.reduce(_ || _))
            ch.paddr                         := Mux1H(
                Seq(
